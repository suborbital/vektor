...
func (rt *Router) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	handler, params, _ := rt.Lookup(r.Method, r.URL.Path)
...
func (rt *Router) mountGroup(group *RouteGroup) {
	for _, r := range group.routeHandlers() {
		...
		rt.Handle(r.Method, r.Path, rt.with(r.Handler))
	}
}
...
// with returns an httprouter.Handle that uses the `inner` gapi.HandleFunc to handle the request
//
